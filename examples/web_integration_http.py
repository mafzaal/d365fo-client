...
async def demo_basic_operations():
    ...
    async with D365FOWebClient() as client:
        try:
            ...
            env_result = await client.call_tool(
            ...
            if env_result.get("success"):
                ...
                print(f"   Application Version: {env_info.get('application_version', 'N/A')}")
            ...
            if search_result.get("success"):
                ...
                for entity in entities:
                    print(f"   - {entity.get('name', 'N/A')}: {entity.get('label_text', 'N/A')}")
            else:
                print("   ❌ Entity search failed")
                print(f"   Error: {search_result.get('error_message', 'Unknown error')}")
            ...
            if search_result.get("success") and search_result.get("entities"):
                ...
                if entity_name:
                    ...
                    if schema_result.get("success"):
                        ...
                        if key_props:
                            print(f"   Key fields: {', '.join(p['name'] for p in key_props)}")
                    else:
                        print("   ❌ Schema retrieval failed")
                        print(f"   Error: {schema_result.get('error_message', 'Unknown error')}")
            
            # 6. Access environment status resource
            print("\n6. 📈 Accessing environment status resource...")
            try:
                resource_result = await client.get_resource("d365fo://environment/status")
        ...
        try:
            ...
            if query_result.get("success"):
                ...
                for i, record in enumerate(records, 1):
                    print(f"   {i}. {record.get('CustomerAccount', 'N/A')} - {record.get('Name', 'N/A')} ({record.get('SalesCurrencyCode', 'N/A')})")
            else:
                ...
                if "authentication" in str(query_result.get('error_message', '')).lower():
                    print("   ℹ️  This is expected without proper D365FO environment configuration")
...
async def demo_profile_management():
    ...
    async with D365FOWebClient() as client:
        try:
            ...
            profiles_result = await client.call_tool(
            ...
            if profiles_result.get("success"):
                ...
                print(f"   Error: {profiles_result.get('error_message', 'Unknown error')}")
            ...
            if default_result.get("success"):
                ...
                print(f"   Error: {default_result.get('error_message', 'Unknown error')}")
...
async def main():
    ...
    try:
        ...
        print(f"\n❌ Connection Error: {e}")
        print("\n🔧 Troubleshooting:")
        print("   1. Ensure FastMCP server is running: d365fo-fastmcp-server --transport http --port 8000")
