...
async def demonstrate_enumeration_tools():
    ...
    try:
        ...
        search_args = {"pattern": ".*[Ss]tatus.*", "limit": 5}
        ...
        print(
            f"Found {search_response['totalCount']} enumerations (showing {len(search_response['enumerations'])})"
        )
        ...
        if search_response["enumerations"]:
            ...
            fields_results = await metadata_tools.execute_get_enumeration_fields(
                fields_args
            )
        ...
        currency_args = {"pattern": ".*[Cc]urrency.*", "limit": 3}

        currency_results = await metadata_tools.execute_search_enumerations(
            currency_args
        )
        ...
        invalid_args = {"enumeration_name": "NonExistentEnumeration123456"}

        try:
            error_results = await metadata_tools.execute_get_enumeration_fields(
                invalid_args
            )
