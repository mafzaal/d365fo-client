...
async def example_environment_credentials():
    ...
    if all(
        os.getenv(var)
        for var in ["D365FO_CLIENT_ID", "D365FO_CLIENT_SECRET", "D365FO_TENANT_ID"]
    ):
        credential_manager = CredentialManager()
        try:
            client_id, client_secret, tenant_id = (
                await credential_manager.get_credentials(env_source)
            )
...
async def example_keyvault_credentials():
    ...
    print(
        "Note: Key Vault credential retrieval requires real vault setup and permissions"
    )
...
async def example_authentication_manager_integration():
    ...
    print(
        f"AuthenticationManager created with credential source: {type(credential_source).__name__}"
    )
...
async def main():
    ...
    print("\nNext Steps:")
    print(
        "1. Set environment variables (D365FO_CLIENT_ID, D365FO_CLIENT_SECRET, D365FO_TENANT_ID)"
    )
