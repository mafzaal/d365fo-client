...
async def test_complete_optimizations():
    ...
    async with FOClient(config) as client:
        ...
        entities = await client.get_all_public_entities_with_details(resolve_labels=False)
        entities_duration = (time.time() - start_time) * 1000
        
        print(f"✅ Retrieved {len(entities)} public entities in {entities_duration:.2f}ms")
        ...
        print("-" * 42)
        start_time = time.time()
        enumerations = await client.get_all_public_enumerations_with_details(resolve_labels=False)
        enums_duration = (time.time() - start_time) * 1000
        
        print(f"✅ Retrieved {len(enumerations)} enumerations in {enums_duration:.2f}ms")
        ...
        print(f"   - Public Entities: Single API call instead of {len(entities):,} individual calls")
        print(f"   - Public Enumerations: Single API call instead of {len(enumerations):,} individual calls")
        print(f"   - Total API Calls Saved: {len(entities) + len(enumerations) - 2:,} calls")
