...
async def demonstrate_enhanced_caching():
    ...
    try:
        ...
        if cache_path.exists():
            ...
            logger.info(
                f"  Environments: {stats.get('metadata_environments_count', 0)}"
            )
        ...
        logger.info(
            f"  Total Environments: {stats.get('metadata_environments_count', 0)}"
        )
        ...
        if "version_manager" in stats:
            ...
            logger.info(
                f"  Total References: {vm_stats.get('reference_statistics', {}).get('total_references', 0)}"
            )
            logger.info(
                f"  Average References per Version: {vm_stats.get('reference_statistics', {}).get('avg_references', 0)}"
            )
            logger.info(
                f"  Recent Activity (7 days): {vm_stats.get('recent_activity', {}).get('versions_used_last_7_days', 0)}"
            )
        ...
        try:
            ...
            if hasattr(client, "call_action"):
                result = await client.call_action(
                    "Microsoft.Dynamics.DataEntities.GetInstalledModules",
                ...
                if result and "value" in result:
                    ...
                    logger.info(
                        f"✅ GetInstalledModules returned {len(modules_data)} modules"
                    )
                    ...
                    logger.warning(
                        "⚠️  GetInstalledModules returned empty or invalid result"
                    )
            else:
                logger.info(
                    "ℹ️  Client does not have call_action method - using alternative approach"
                )
            ...
            logger.info(
                "This is expected if the action is not available in this environment"
            )
            ...
            try:
                ...
                logger.error(
                    f"❌ All version detection methods failed: {fallback_error}"
                )
