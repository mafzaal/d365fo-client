...
async def test_query_methods():
    ...
    async with FOClient(config) as client:
        ...
        try:
            ...
            print(f"   Found {len(master_entities_int)} Master entities (integer method)")
            
            if master_entities_int:
                # Show sample
                for i, entity in enumerate(master_entities_int[:3]):
                    print(f"   - {entity.get('EntityName', 'N/A')} (Category: {entity.get('Category', 'N/A')})")
        ...
        try:
            ...
            master_entities_str = await client.query_data_management_entities_by_category(
                categories=['Master']
            )
            print(f"   Found {len(master_entities_str)} Master entities (string method)")
            ...
            if 'master_entities_int' in locals():
                if len(master_entities_int) == len(master_entities_str):
                    ...
                    print(f"   ⚠ Count mismatch: int={len(master_entities_int)} vs str={len(master_entities_str)}")
        ...
        try:
            ...
            # Show distribution
            master_count = sum(1 for e in multi_entities if e.get('Category') == 'Master')
            transaction_count = sum(1 for e in multi_entities if e.get('Category') == 'Transaction')
            print(f"   Distribution: {master_count} Master, {transaction_count} Transaction")
        ...
        try:
            ...
            non_shared_entities = await client.query_data_management_entities_by_category(
                categories=['Master'],
                is_shared=False
            )
            print(f"   Found {len(non_shared_entities)} non-shared Master entities")
            print(f"   Total Master entities check: {len(shared_entities) + len(non_shared_entities)}")
        ...
        try:
            ...
            gl_entities = await client.query_data_management_entities_by_category(
                categories=['Master', 'Configuration'],
                modules=['GeneralLedger']
            )
            print(f"   Found {len(gl_entities)} General Ledger Master/Configuration entities")
            
            if gl_entities:
                for i, entity in enumerate(gl_entities[:3]):
                    print(f"   - {entity.get('EntityName', 'N/A')} (Category: {entity.get('Category', 'N/A')}, Module: {entity.get('Modules', 'N/A')})")
        ...
        try:
            ...
            all_categories = ['Master', 'Configuration', 'Transaction', 'Reference', 'Document', 'Parameters']
            category_counts = {}
        ...
        try:
            ...
            print(f"   Found {len(complex_entities)} shared Master/Configuration GeneralLedger entities")
