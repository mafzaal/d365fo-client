...
async def test_public_entities_parsing():
    ...
    async with FOClient(config) as client:
        ...
        entity = await client.get_public_entity_info("AbsenceCode", resolve_labels=False)
        
        if entity:
            print(f"\n🔍 Testing entity: {entity.name}")
            print(f"   Entity Set Name: {entity.entity_set_name}")
            print(f"   Properties Count: {len(entity.properties)}")
            print(f"   Navigation Properties Count: {len(entity.navigation_properties)}")
            ...
            if entity.navigation_properties:
                ...
                for nav_prop in entity.navigation_properties:
                    print(f"   - {nav_prop.name} -> {nav_prop.related_entity} ({nav_prop.cardinality})")
                    if nav_prop.constraints:
                        print(f"     Constraints:")
                        for constraint in nav_prop.constraints:
                            print(f"       {constraint.constraint_type}: {constraint.property} -> {constraint.referenced_property}")
        ...
        entity2 = await client.get_public_entity_info("AbsenceCodeGroup", resolve_labels=False)
        
        if entity2:
            print(f"\n🔍 Testing entity: {entity2.name}")
            print(f"   Navigation Properties Count: {len(entity2.navigation_properties)}")
