...
class SequenceAnalysisDemo:
    ...
    async def analyze_manual_sequences(self) -> Dict[str, Any]:
        ...
        manual_sequences = await self.client.get_data("SequenceV2Tables", 
        ...
        sequence_codes = [seq['NumberSequenceCode'] for seq in manual_sequences['value']]
        references = []
        
        for code in sequence_codes:
            ref_data = await self.client.get_data("NumberSequencesV2References",
        ...
        result = {
            ...
            "insights": self._analyze_manual_sequence_patterns(manual_sequences['value'], references)
        }
    ...
    async def analyze_customer_sequences(self) -> Dict[str, Any]:
        ...
        customer_refs = await self.client.get_data("NumberSequencesV2References",
        ...
        sequence_codes = list(set([ref['NumberSequenceCode'] for ref in customer_refs['value']]))
        sequence_details = []
        
        for code in sequence_codes:
            seq_data = await self.client.get_data("SequenceV2Tables",
                                                filter=f"NumberSequenceCode eq '{code}'",
        ...
        result = {
            ...
            "insights": self._analyze_customer_sequence_patterns(customer_refs['value'], sequence_details)
        }
    ...
    async def analyze_sequence_health(self) -> Dict[str, Any]:
        ...
        # Get all sequences
        all_sequences = await self.client.get_data("SequenceV2Tables",
                                                 select="NumberSequenceCode,Name,NextRec,LargestValue,SmallestValue,Format,InUse,Stopped,Manual,Company")
        
        # Get all references
        all_references = await self.client.get_data("NumberSequencesV2References",
                                                   select="DataTypeName,NumberSequenceCode,Company")
        
        health_analysis = self._perform_health_analysis(all_sequences['value'], all_references['value'])
    ...
    def _analyze_manual_sequence_patterns(self, sequences: List[Dict], references: List[Dict]) -> Dict[str, Any]:
        """Analyze patterns in manual sequences."""
        active_manual = [s for s in sequences if s.get('InUse') and not s.get('Stopped')]
    ...
    def _analyze_customer_sequence_patterns(self, references: List[Dict], sequences: List[Dict]) -> Dict[str, Any]:
    ...
    def _perform_health_analysis(self, sequences: List[Dict], references: List[Dict]) -> Dict[str, Any]:
        ...
        unused = [s for s in sequences if s['NumberSequenceCode'] not in referenced_codes]
        
        # Orphaned references (no sequence definition)
        sequence_codes = set([s['NumberSequenceCode'] for s in sequences])
        orphaned_refs = [r for r in references if r['NumberSequenceCode'] not in sequence_codes]
        ...
        return {
            ...
            "health_score": self._calculate_health_score(len(sequences), len(near_limit), len(stopped), len(unused), len(orphaned_refs))
        }
    
    def _calculate_health_score(self, total: int, near_limit: int, stopped: int, unused: int, orphaned: int) -> int:
    ...
    def _print_manual_sequence_summary(self, result: Dict[str, Any]):
        ...
        print(f"Entities Using Manual Sequences: {result['insights']['entities_using_manual']}")
        
        print("\nTop Manual Sequences:")
        for seq in result['sequences'][:5]:
            status = "🟢 Active" if seq.get('InUse') and not seq.get('Stopped') else "🔴 Inactive"
            print(f"  {seq['NumberSequenceCode']} ({seq.get('Company', 'N/A')}) - {seq.get('Name', 'N/A')} - {status}")
    ...
    def _print_customer_sequence_summary(self, result: Dict[str, Any]):
        ...
        print(f"Manual Customer Sequences: {result['insights']['manual_customer_sequences']}")
    ...
    def _print_health_analysis(self, result: Dict[str, Any]):
        ...
        print(f"Overall Health Score: {score}/100 {'🟢' if score >= 80 else '🟡' if score >= 60 else '🔴'}")
        ...
        if health['near_limit_sequences']:
            ...
            for seq in health['near_limit_sequences'][:5]:
                print(f"  {seq['NumberSequenceCode']} ({seq.get('Company', 'N/A')}) - {seq['utilization_percent']}% utilized")
        
        if health['stopped_sequences']:
            print("\n🛑 Stopped Sequences:")
            for seq in health['stopped_sequences'][:5]:
                print(f"  {seq['NumberSequenceCode']} ({seq.get('Company', 'N/A')}) - {seq.get('Name', 'N/A')}")
    ...
    async def run_comprehensive_demo(self):
        ...
        try:
            ...
            print(f"Connected to D365FO: {version.get('ApplicationVersion', 'Unknown')}")
            ...
            print(f"Manual Sequences Analyzed: {manual_result['total_manual_sequences']}")
            print(f"Customer Sequences Analyzed: {customer_result['unique_sequences']}")
            print(f"Overall Health Score: {health_result['health_metrics']['health_score']}/100")
...
async def main():
    ...
    try:
        ...
        print("Use this template with AI assistants for comprehensive sequence analysis.")
