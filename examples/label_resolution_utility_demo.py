...
async def main():
    ...
    try:
        ...
        if entity:
            ...
            print(f"Properties with labels: {len([p for p in entity.properties if p.label_id])}")
            ...
            print(f"First property with label: {entity.properties[0].name} -> {entity.properties[0].label_text}")
            print()
        ...
        entities = await cache.search_data_entities(pattern="Customer", data_service_enabled=True)
        if entities:
        ...
        if enums:
            enum_info = await cache.get_public_enumeration_info(enums[0].name, resolve_labels=False)
            if enum_info:
                print(f"Enumeration: {enum_info.name}")
                print(f"Label ID: {enum_info.label_id}")
                print(f"Label Text (before): {enum_info.label_text}")
                print(f"Members with labels: {len([m for m in enum_info.members if m.label_id])}")
                ...
                if enum_info.members:
                    print(f"First member: {enum_info.members[0].name} -> {enum_info.members[0].label_text}")
                print()
