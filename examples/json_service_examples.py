...
async def json_service_with_parameters_example():
    ...
    async with FOClient(config) as client:
        ...
        parameters = {"start": start_time.isoformat(), "end": end_time.isoformat()}
        ...
        if response.success:
            ...
            if response.data:
                total_duration = sum(
                    float(record.get("duration_ms", 0)) for record in response.data
                )
...
async def custom_service_example():
    ...
    async with FOClient(config) as client:
        ...
        # Optional parameters
        parameters = {"parameter1": "value1", "parameter2": 12345, "parameter3": True}

        print(f"  Service: {service_group}/{service_name}/{operation_name}")
        print(
            f"  Endpoint: /api/services/{service_group}/{service_name}/{operation_name}"
        )
        ...
        try:
            ...
            if response.success:
                print(
                    f"  ✓ Custom service call successful (HTTP {response.status_code})"
                )
        ...
        print(
            "  Note: Replace service details with actual values for your custom service"
        )
...
async def performance_example():
    ...
    async with FOClient(config) as client:
        ...
        for operation in operations:
            ...
            print(
                f"  {operation}: {result['duration_ms']}ms, "
        ...
        print(
            f"  Summary: {len(operations)} operations in {total_time:.2f}ms "
