...
async def test_enhanced_client():
    ...
    async with FOClient(config) as client:
        ...
        entities_no_cache = await client.search_entities(
            "Customer", use_cache_first=False
        )
        print(
            f"Fallback entity search results: {len(entities_no_cache)} entities found"
        )
        ...
        if entities:
            ...
            entity_info = await client.get_entity_info(
                entity_name, use_cache_first=True
            )
            if entity_info:
        ...
        public_entities = await client.search_public_entities(
            "Customer", use_cache_first=True
        )
