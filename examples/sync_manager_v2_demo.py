...
async def demo_sync_manager_v2():
    ...
    def progress_callback(progress):
        logger.info(
            f"Sync Progress: {progress.phase} - {progress.current_operation} "
    ...
    try:
        ...
        logger.info(
            f"Global version ID: {global_version_id}, Sync needed: {sync_needed}"
        )
        ...
        entities = await cache.get_data_entities(
            global_version_id=global_version_id, data_service_enabled=True
        )
        ...
        for i, entity in enumerate(entities[:5]):
            logger.info(
                f"  {i+1}. {entity.name} -> {entity.public_entity_name} ({entity.entity_category})"
            )
        ...
        if enum_info:
            logger.info(
                f"Enumeration '{enum_info.name}' has {len(enum_info.members)} members"
            )
    ...
    try:
        ...
        if global_version_id and sync_needed:
            ...
            result = await sync_manager.sync_metadata(
                global_version_id, SyncStrategy.ENTITIES_ONLY
            )

            logger.info(
                f"Fast sync completed: {result.entity_count} entities in {result.duration_ms}ms"
            )
