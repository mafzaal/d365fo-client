...
async def main():
    ...
    async with FOClient(config) as client:
        ...
        metadata_cache = MetadataCacheV2(
            cache_dir, config.base_url, client.metadata_api_ops
        )
        ...
        sync_needed, global_version_id = await metadata_cache.check_version_and_sync(
            client.metadata_api_ops
        )
        ...
        customer_entities = await metadata_cache.get_data_entities(
        ...
        master_entities = await metadata_cache.get_data_entities(
        ...
        print(
            f"5 data entity searches: {lookup_time:.2f}ms ({lookup_time/5:.2f}ms avg)"
        )
