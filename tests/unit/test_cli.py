...
class TestCLIManager:
    ...
    async def test_demo_mode(self):
        ...
        with patch(
            "d365fo_client.main.example_usage", new_callable=AsyncMock
        ) as mock_example:
    ...
    async def test_config_list_empty(self):
        ...
        args = argparse.Namespace(
            demo=False, command="config", config_subcommand="list", output="json"
        )

        # Mock the config manager to return empty profiles
        with (
            patch.object(cli_manager.config_manager, "list_profiles", return_value={}),
            patch.object(
                cli_manager.config_manager, "get_default_profile", return_value=None
            ),
        ):
            # Capture stdout
    ...
    async def test_missing_base_url(self):
        ...
        args = argparse.Namespace(
            demo=False, command="test", output="json", base_url=None, profile=None
        )
...
class TestArgumentParser:
    ...
    def test_version_command(self):
        ...
        args = parser.parse_args(
            ["--base-url", "https://test.com", "version", "--application"]
        )
    ...
    def test_test_command(self):
        ...
        args = parser.parse_args(
            ["--base-url", "https://test.com", "test", "--odata-only"]
        )
