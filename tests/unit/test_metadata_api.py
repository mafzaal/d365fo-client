...
class TestPublicEntitiesAPI:
    ...
    @pytest.mark.asyncio
    async def test_get_public_entity_info_with_properties(
        self, metadata_api_ops, mock_session_manager, mock_label_ops
    ):
...
class TestPublicEnumerationsAPI:
    ...
    @pytest.mark.asyncio
    async def test_search_public_enumerations(
        self, metadata_api_ops, mock_session_manager
    ):
        ...
        mock_response.status = 200
        mock_response.json.return_value = {
            "value": [{"Name": "PaymentType", "LabelId": "@PAY123"}]
        }
    ...
    @pytest.mark.asyncio
    async def test_get_public_enumeration_info_with_members(
        self, metadata_api_ops, mock_session_manager, mock_label_ops
    ):
