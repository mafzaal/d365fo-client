...
class TestCacheStatisticsEnvironmentScoping:
    ...
    async def test_cache_statistics_are_environment_scoped(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            module1 = ModuleVersionInfo(
                name="Module 1", version="1.0", module_id="module1", 
                publisher="Test", display_name="Test Module 1"
            )
            module2 = ModuleVersionInfo(
                name="Module 2", version="1.0", module_id="module2", 
                publisher="Test", display_name="Test Module 2"
            )
            ...
            assert stats1['data_entities_count'] == 2, f"Cache1 should have 2 entities, got {stats1['data_entities_count']}"
            assert stats2['data_entities_count'] == 1, f"Cache2 should have 1 entity, got {stats2['data_entities_count']}"
            ...
            assert stats1['current_version']['global_version_id'] != stats2['current_version']['global_version_id']
    ...
    async def test_database_statistics_methods_compatibility(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            env_stats = await cache.database.get_environment_database_statistics(cache._environment_id)
    ...
    async def test_version_manager_environment_scoping(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            env_stats = await cache.version_manager.get_environment_version_statistics(cache._environment_id)
