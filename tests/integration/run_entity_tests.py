...
class EntityTestRunner:
    ...
    async def run_discovery_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            public_entities_result = await self.client.get_public_entities(QueryOptions(top=50))
            ...
            for i, entity_info in enumerate(public_entities[:25], 1):  # Limit to 25 for demo
                ...
                try:
                    ...
                    entity_schema = await self.client.get_public_entity_info(entity_name, resolve_labels=False)
                    ...
                    if len(key_properties) > 1:  # Composite key found!
                        ...
                        print(f"    ✅ Composite key found! {len(key_properties)} keys with types: {list(key_data_types)}")
                        
                        # Stop after finding several examples
                        if len(results["composite_key_entities"]) >= 10:
                            print(f"    🎯 Found {len(results['composite_key_entities'])} composite key entities, stopping discovery")
                            break
    ...
    async def run_validation_tests(self, composite_entities: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for i, entity in enumerate(composite_entities[:5], 1):  # Test first 5
            ...
            try:
                ...
                is_valid, schema, error = await self.mixin._validate_entity_schema_and_keys(
                    entity_name, key_field_names, dummy_values
                )
                test_result["tests"]["schema_validation"] = is_valid
                test_result["tests"]["schema_error"] = error.get("error", "") if error else ""
                ...
                if schema:
                    for key_field in key_fields:
                        ...
                        try:
                            ...
                            serialized = self.mixin._serialize_odata_value(test_value, data_type, type_name)
        ...
        return {
            ...
            "summary": {"passed": passed, "failed": failed, "total": len(validation_results)}
        }
    ...
    def print_summary(self, discovery_results: Dict[str, Any], validation_results: Dict[str, Any]):
        ...
        print(f"  📋 Total entities checked: {discovery_results['total_entities_checked']}")
        print(f"  🔑 Composite key entities found: {len(discovery_results['composite_key_entities'])}")
        print(f"  🎭 Mixed data type entities: {len(discovery_results['mixed_type_entities'])}")
        print(f"  📊 Data types discovered: {len(discovery_results['data_types_found'])}")
        ...
        if discovery_results["composite_key_entities"]:
            ...
            for entity in discovery_results["composite_key_entities"][:5]:  # Show first 5
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run entity validation and OData serialization tests")
    parser.add_argument("--sandbox", action="store_true", help="Run against sandbox environment")
    parser.add_argument("--live", action="store_true", help="Run against live environment")
    parser.add_argument("--discovery-only", action="store_true", help="Run discovery tests only")
    ...
    if not base_url:
        ...
        print("Set D365FO_SANDBOX_BASE_URL or D365FO_LIVE_BASE_URL environment variable")
        sys.exit(1)
