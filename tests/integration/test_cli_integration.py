...
class TestCLIArgumentValidation:
    ...
    def test_cli_entity_get_missing_entity_name(self):
        ...
        result = subprocess.run(
            ['uv', 'run', 'd365fo-client', '--base-url', 'http://localhost:8000', 'entity', 'get'],
            capture_output=True,
            text=True,
            cwd=str(Path(__file__).parent.parent.parent)
        )
    ...
    def test_cli_entity_create_missing_data(self):
        ...
        result = subprocess.run(
            ['uv', 'run', 'd365fo-client', '--base-url', 'http://localhost:8000', 
             'entity', 'create', 'Customers'],
            capture_output=True,
            text=True,
            cwd=str(Path(__file__).parent.parent.parent)
        )
...
class TestCLIFileOperations:
    ...
    def test_cli_create_with_invalid_file(self):
        ...
        result = subprocess.run(
            ['uv', 'run', 'd365fo-client', '--base-url', 'http://localhost:8000',
             'entity', 'create', 'Customers', '--file', '/nonexistent/file.json'],
            capture_output=True,
            text=True,
            cwd=str(Path(__file__).parent.parent.parent)
        )
    ...
    def test_cli_create_with_invalid_json_file(self):
        ...
        try:
            result = subprocess.run(
                ['uv', 'run', 'd365fo-client', '--base-url', 'http://localhost:8000',
                 'entity', 'create', 'Customers', '--file', temp_file],
                capture_output=True,
                text=True,
                cwd=str(Path(__file__).parent.parent.parent)
            )
    ...
    def test_cli_create_with_valid_json_file_structure(self):
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                assert 'json' not in result.stdout.lower() or 'parse' not in result.stdout.lower()
        finally:
            os.unlink(temp_file)
...
class TestCLILogicIntegration:
    ...
    def test_cli_subcommand_discovery(self):
        ...
        expected_commands = ['entity', 'metadata', 'version', 'test', 'config', 'action']
...
class TestCLIErrorMessages:
    ...
    def test_cli_invalid_json_data_error(self):
        ...
        result = subprocess.run(
            ['uv', 'run', 'd365fo-client', '--base-url', 'http://localhost:8000',
             'entity', 'create', 'Customers', '--data', 'invalid json {'],
            capture_output=True,
            text=True,
            cwd=str(Path(__file__).parent.parent.parent)
        )
