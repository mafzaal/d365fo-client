...
class TestSandboxVersionMethods:
    ...
    async def test_version_consistency(self, sandbox_client: FOClient):
        ...
        assert all(
            len(v) > 0 for v in [app_version, platform_version, app_build_version]
        )
...
class TestSandboxDataOperations:
    ...
    async def test_get_available_entities(self, sandbox_client: FOClient):
        ...
        assert (
            len(successful_entities) > 0
        ), f"No common entities accessible: {common_entities}"
    ...
    async def test_odata_query_options(self, sandbox_client: FOClient):
        ...
        try:
            ...
            result_all = await sandbox_client.get_entities(
                "Companies", QueryOptions(top=5)
            )

            if "value" in result_all and len(result_all["value"]) > 0:
                # Test top parameter
                result_top1 = await sandbox_client.get_entities(
    ...
    async def test_concurrent_operations(self, sandbox_client: FOClient):
        ...
        for i, result in enumerate(results):
            assert not isinstance(
                result, Exception
            ), f"Concurrent operation {i} failed: {result}"


@skip_if_not_level("sandbox")
class TestSandboxJsonServices:
    """Test JSON service operations against sandbox environment."""

    @pytest.mark.asyncio
    async def test_post_json_service_basic_call(self, sandbox_client: FOClient):
        """Test basic JSON service call to GetAxSqlExecuting."""
    ...
    async def test_call_json_service_with_request_object(
        ...
        response = await sandbox_client.call_json_service(request)
    ...
    @pytest.mark.asyncio
    async def test_post_json_service_error_handling(self, sandbox_client: FOClient):
        """Test JSON service error handling with invalid operation."""
        response = await sandbox_client.post_json_service(
            service_group="SysSqlDiagnosticService",
            service_name="SysSqlDiagnosticServiceOperations",
        ...
        assert response.status_code >= 400  # Should be an HTTP error code
    ...
    async def test_post_json_service_invalid_service_group(
        ...
        response = await sandbox_client.post_json_service(
            ...
            service_name="NonExistentOperations",
            operation_name="NonExistentOperation",
        )
