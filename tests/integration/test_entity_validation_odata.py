...
from d365fo_client.models import QueryOptions, PublicEntityInfo
...
class TestEntityValidation:
    ...
    async def test_resolve_entity_name_variations(self, sandbox_client: FOClient):
        ...
        for test_case in test_cases:
            ...
            assert resolved is not None, f"Failed to resolve entity name: {test_case['name']}"
            
            # Test 2: Resolve using public collection name
            if test_case["public_collection"]:
                resolved = await mixin._resolve_entity_name(test_case["public_collection"])
                assert resolved is not None, f"Failed to resolve public collection name: {test_case['public_collection']}"
            
            # Test 3: Resolve using public entity name
            if test_case["public_entity"] and test_case["public_entity"] != test_case["name"]:
                resolved = await mixin._resolve_entity_name(test_case["public_entity"])
                assert resolved is not None, f"Failed to resolve public entity name: {test_case['public_entity']}"
    ...
    async def test_validate_entity_for_query(self, sandbox_client: FOClient):
        ...
        for entity in odata_entities[:3]:  # Test first 3
            ...
            if public_collection and public_collection != entity_name:
                ...
                assert is_valid, f"Public collection {public_collection} should be valid for queries"
...
class TestCompositeKeyDiscovery:
    ...
    async def test_find_entities_with_composite_keys(self, sandbox_client: FOClient):
        ...
        public_entities_result = await sandbox_client.get_public_entities(QueryOptions(top=50))
        ...
        for entity_info in public_entities[:20]:  # Check first 20 entities
            ...
            try:
                ...
                entity_schema = await sandbox_client.get_public_entity_info(entity_name, resolve_labels=False)
                if not entity_schema:
                    continue
                
                # Find key properties
                key_properties = [prop for prop in entity_schema.properties if prop.is_key]
                ...
                if len(key_properties) > 1:  # Composite key
                    ...
                    logger.info(f"Found composite key entity: {entity_name} with {len(key_properties)} keys")
        ...
        assert len(composite_key_entities) > 0, "Should find at least one entity with composite keys"
        ...
        for entity in composite_key_entities:
            ...
            for key_field in entity["key_fields"]:
                print(f"    🔑 {key_field['name']}: {key_field['data_type']} ({key_field['type_name']})")
    ...
    async def test_composite_key_data_types(self, sandbox_client: FOClient):
        ...
        if not hasattr(pytest, 'composite_key_entities') or not pytest.composite_key_entities:
        ...
        assert len(data_types_found) > 1, f"Expected multiple data types, found: {data_types_found}"
...
class TestODataSerialization:
    ...
    async def test_serialize_real_entity_key_fields(self, sandbox_client: FOClient):
        ...
        if serialization_tests:
            ...
            assert success_rate > 0.8, f"Low success rate: {success_rate:.2%} ({len(failed_tests)} failures)"
    ...
    async def test_build_validated_key_dict(self, sandbox_client: FOClient):
        ...
        for field_name in key_fields:
            ...
            assert isinstance(key_dict[field_name], str)  # Should be serialized as string
    ...
    async def test_validate_entity_schema_and_keys(self, sandbox_client: FOClient):
        ...
        for entity in data_entities:
            ...
            try:
                ...
                entity_schema = await sandbox_client.get_public_entity_info(resolved_name, resolve_labels=False)
                if not entity_schema:
                    continue
                ...
                is_valid, schema, error_details = await mixin._validate_entity_schema_and_keys(
                ...
                if len(key_fields) > 1:
                    ...
                    is_valid_incomplete, _, error_details_incomplete = await mixin._validate_entity_schema_and_keys(
                        entity_name, incomplete_fields, incomplete_values
                    )
                    
                    assert not is_valid_incomplete, f"Validation should fail for incomplete keys on {entity_name}"
                    assert error_details_incomplete is not None, "Should have error details for incomplete keys"
        ...
        if failed_validations:
            ...
            for result in failed_validations:
                print(f"  {result['entity_name']}: {result.get('error_details', 'Unknown error')}")
        
        # Should have at least some successful validations
        assert len(successful_validations) > 0, "Should have at least one successful schema validation"
...
class TestIntegratedCRUDValidation:
    ...
    async def test_crud_validation_integration(self, sandbox_client: FOClient):
        ...
        invalid_exists = await mixin._validate_entity_exists("CompletelyInvalidEntity9999")
        assert not invalid_exists, "Invalid entity should not exist"
        
        invalid_resolved = await mixin._resolve_entity_name("CompletelyInvalidEntity9999")
        assert invalid_resolved is None, "Invalid entity should not resolve"
        
        invalid_query_valid = await mixin._validate_entity_for_query("CompletelyInvalidEntity9999")
