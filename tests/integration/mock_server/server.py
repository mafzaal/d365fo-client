...
class D365MockServer:
            ...
            "PublicEnumerations": [
                {
                    ...
                    "LabelId": "@SYS9001",
                    "Members": [
                        {"Name": "Customer", "Value": 0, "LabelId": "@SYS9002"},
                        {"Name": "Vendor", "Value": 1, "LabelId": "@SYS9003"},
                    ],
                }
            ],
        }
    ...
    async def setup_routes(self):
        ...
        self.app.router.add_post(
            "/data/{entity}/Microsoft.Dynamics.DataEntities.{action}",
            self.handle_bound_action,
        )
        self.app.router.add_post(
            "/data/Microsoft.Dynamics.DataEntities.{action}", self.handle_unbound_action
        )

        # Metadata API endpoints
        self.app.router.add_get(
            "/Metadata/DataEntities", self.handle_metadata_data_entities
        )
        self.app.router.add_get(
            "/Metadata/DataEntities({entity_name})", self.handle_metadata_data_entity
        )
        self.app.router.add_get(
            "/Metadata/PublicEntities", self.handle_metadata_public_entities
        )
        self.app.router.add_get(
            "/Metadata/PublicEntities({entity_name})",
            self.handle_metadata_public_entity,
        )
        self.app.router.add_get(
            "/Metadata/PublicEnumerations", self.handle_metadata_public_enumerations
        )
        self.app.router.add_get(
            "/Metadata/PublicEnumerations({enum_name})",
            self.handle_metadata_public_enumeration,
        )
        ...
        self.app.router.add_get(
            "/Metadata/Labels(Id={label_id},Language={language})",
            self.handle_label_by_id,
        )
  ...
  <edmx:DataServices>
    ...
    async def handle_entity_collection(self, request: Request) -> Response:
        ...
        if entity not in self.entities_data:
            return web.json_response(
                {"error": {"message": f"Entity set {entity} not found"}}, status=404
            )
        ...
        if "$filter" in query_params:
            ...
            if "contains" in filter_expr.lower():
                ...
                entities = [
                    e
                    for e in entities
                    if any(str(v).lower().find("test") >= 0 for v in e.values())
                ]
    ...
    async def handle_entity_by_key(self, request: Request) -> Response:
        ...
        return web.json_response(
            {"error": {"message": f"Entity with key {key} not found"}}, status=404
        )
    ...
    async def handle_bound_action(self, request: Request) -> Response:
        ...
        if entity == "DataManagementEntities":
            if action == "GetApplicationVersion":
                ...
                return web.json_response(
                    {"value": self.version_info["application_build"]}
                )

        return web.json_response(
            {"error": {"message": f"Action {action} not found for entity {entity}"}},
            status=404,
        )
    ...
    async def handle_unbound_action(self, request: Request) -> Response:
        ...
        action = request.match_info["action"]
        return web.json_response(
            {"error": {"message": f"Unbound action {action} not found"}}, status=404
        )
    ...
    async def handle_metadata_data_entity(self, request: Request) -> Response:
        ...
        return web.json_response(
            {"error": {"message": f"DataEntity {entity_name} not found"}}, status=404
        )
    ...
    async def handle_metadata_public_entity(self, request: Request) -> Response:
        ...
        return web.json_response(
            {"error": {"message": f"PublicEntity {entity_name} not found"}}, status=404
        )
    ...
    async def handle_metadata_public_enumeration(self, request: Request) -> Response:
        ...
        return web.json_response(
            {"error": {"message": f"PublicEnumeration {enum_name} not found"}},
            status=404,
        )
    ...
    async def handle_labels(self, request: Request) -> Response:
        ...
        if "labelId" in query_params:
            ...
            if label_id in self.labels:
                ...
                return web.json_response(
                    {"error": {"message": f"Label {label_id} not found"}}, status=404
                )
