    ...
    try:
        ...
        try:
            async with session.get(odata_url) as response:
                ...
                # Display key headers
                key_headers = [
                    'content-type', 'odata-version', 'server', 'cache-control',
                    'x-ms-environment-id', 'x-ms-tenant-id', 'x-powered-by'
                ]
        ...
        try:
            async with session.get(metadata_url, params={"$top": 1}) as response:
                ...
                key_headers = [
                    ...
                    'x-ms-environment-id', 'x-ms-request-id', 'date'
                ]
        ...
        print("   - AZURE_CLIENT_ID, AZURE_CLIENT_SECRET, AZURE_TENANT_ID (if not using default credentials)")
