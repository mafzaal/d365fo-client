...
async def test_sync_entities_count():
    ...
    try:
        ...
        if len(sync_entities) > 10:
            ...
            if sync_needed:
                logger.info(f"Metadata out of date, detected {detected_version_id}). Starting sync...")
            # Start a sync session and monitor its progress
            session_id = await client.sync_session_manager.start_sync_session(detected_version_id)
