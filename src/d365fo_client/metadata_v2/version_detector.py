...
class ModuleVersionDetector:
    ...
    async def get_environment_version(self, use_cache: bool = True) -> VersionDetectionResult:
...
def compute_version_signature(modules: List[ModuleVersionInfo]) -> str:
    ...
    signature_modules = sorted_modules[:5] + sorted_modules[-5:] if len(sorted_modules) > 10 else sorted_modules
...
def validate_modules_consistency(modules: List[ModuleVersionInfo]) -> Dict[str, Any]:
    ...
    for module in modules:
        ...
        major_version = module.version.split('.')[0] if '.' in module.version else module.version
        if major_version not in version_groups:
            version_groups[major_version] = []
        version_groups[major_version].append(module.module_id)
    
    validation['statistics']['version_groups'] = {k: len(v) for k, v in version_groups.items()}
