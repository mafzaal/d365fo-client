...
from ..models import ModuleVersionInfo, EnvironmentVersionInfo, GlobalVersionInfo
...
class GlobalVersionManager:
    ...
    async def register_environment_version(
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            cursor = await db.execute(
                "SELECT id FROM global_versions WHERE modules_hash = ?",
                (modules_hash,)
            )
            ...
            await self._link_environment_to_version(db, environment_id, global_version_id)
    ...
    async def _get_global_version_modules(
        ...
        for row in await cursor.fetchall():
            modules.append(ModuleVersionInfo(
                module_id=row[0],
                name=row[1],
                version=row[2],
                publisher=row[3],
    ...
    async def get_global_version_info(self, global_version_id: int) -> Optional[GlobalVersionInfo]:
    ...
    async def find_compatible_versions(
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            for row in await cursor.fetchall():
                ...
                if version_info:
                    if exact_match or self._is_compatible(modules, version_info.modules):
    ...
    async def get_version_statistics(self) -> Dict[str, Any]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            cursor = await db.execute("SELECT COUNT(DISTINCT environment_id) FROM environment_versions")
