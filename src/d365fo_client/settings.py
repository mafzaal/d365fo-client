...
class D365FOSettings(BaseSettings):
    ...
    verify_ssl: bool = Field(
        default=True, description="Verify SSL certificates", alias="D365FO_VERIFY_SSL"
    )
    ...
    cache_dir: Optional[str] = Field(
        default=None, description="General cache directory", alias="D365FO_CACHE_DIR"
    )
    ...
    use_label_cache: bool = Field(
        default=True, description="Enable label caching", alias="D365FO_LABEL_CACHE"
    )
    ...
    def has_mcp_auth_credentials(self) -> bool:
        ...
        return all(
            [
                self.mcp_auth_client_id,
                self.mcp_auth_client_secret,
                self.mcp_auth_tenant_id,
            ]
        )
    ...
    def get_startup_mode(
        self,
    ) -> Literal["profile_only", "default_auth", "client_credentials"]:
        """Determine startup mode based on configuration."""
        if (
            self.base_url
            and self.base_url != "https://usnconeboxax1aos.cloud.onebox.dynamics.com"
        ):
