...
def create_default_profile_if_needed(
    profile_manager: "ProfileManager", config: Dict
) -> Optional[bool]:
    ...
    try:
        ...
        if base_url.startswith("https://usnconeboxax1aos.cloud.onebox.dynamics.com"):
            logger.warning(
                "D365FO_BASE_URL is set to the default onebox URL - please set it to your actual environment URL"
            )
            return False
        ...
        client_secret = (
            default_environment.get("client_secret") or settings.client_secret
        )
...
def migrate_legacy_config(profile_manager: "ProfileManager") -> bool:
    ...
    try:
        ...
        if "default_profile" in config_data:
            try:
                ...
                if default_profile_name in migrated_profiles:
                    ...
                    logger.info(
                        f"Migrated default profile setting: {default_profile_name}"
                    )
            except Exception as e:
        ...
        logger.info(
            f"Migration completed: {successful_migrations}/{total_profiles} profiles migrated successfully"
        )

        if successful_migrations > 0:
            logger.info(f"Original config backed up to: {backup_path}")
            logger.info(
                "You can remove the backup file once you've verified the migration worked correctly"
            )
            return True
        else:
...
def _is_legacy_config_format(config_data: Dict[str, Any]) -> bool:
    ...
    for profile_name, profile_data in profiles.items():
        ...
        if "verify_ssl" not in profile_data:
            logger.debug(
                f"Legacy format detected: profile {profile_name} missing verify_ssl"
            )
            return True

        # Check for auth_mode without use_default_credentials (legacy pattern)
        if (
            "auth_mode" in profile_data
            and "use_default_credentials" not in profile_data
        ):
            logger.debug(
                f"Legacy format detected: profile {profile_name} has auth_mode but missing use_default_credentials"
            )
            return True
        ...
        if "cache_dir" in profile_data and "metadata_cache_dir" not in profile_data:
            logger.debug(
                f"Legacy format detected: profile {profile_name} uses cache_dir instead of metadata_cache_dir"
            )
            return True
        ...
        if credential_source is not None and isinstance(credential_source, dict):
            ...
            if not all(field in credential_source for field in required_fields):
                logger.debug(
                    f"Legacy format detected: profile {profile_name} has incomplete credential_source"
                )
                return True
...
def _migrate_legacy_profile_data(profile_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if auth_mode and "use_default_credentials" not in migrated_data:
        if auth_mode == "default":
            ...
            logger.debug(
                "Migrated auth_mode='client_credentials' to use_default_credentials=False"
            )
        else:
            # Unknown auth_mode, default to True for safety
            migrated_data["use_default_credentials"] = True
            logger.warning(
                f"Unknown auth_mode '{auth_mode}', defaulting to use_default_credentials=True"
            )
