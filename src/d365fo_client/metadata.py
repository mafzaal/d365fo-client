...
from .models import ActionInfo, PublicEntityInfo
...
class MetadataManager:
    ...
    def search_entities(self, pattern: str = "") -> List[str]:
        ...
        flags = re.IGNORECASE
        return sorted(
            [name for name in entities.keys() if re.search(pattern, name, flags)]
        )
    ...
    def search_actions(self, pattern: str = "") -> List[str]:
        ...
        flags = re.IGNORECASE
        return sorted(
            [name for name in actions.keys() if re.search(pattern, name, flags)]
        )
