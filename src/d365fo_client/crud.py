...
class CrudOperations:
    ...
    async def get_entity(self, entity_name: str, key: str, 
        ...
        async with session.get(url) as response:
            if response.status == 200:
                ...
                error_text = await response.text()
                raise Exception(f"GET {entity_name}({key}) failed: {response.status} - {error_text}")
    ...
    async def delete_entity(self, entity_name: str, key: str) -> bool:
        ...
        async with session.delete(url) as response:
            if response.status in [200, 204]:
                ...
                error_text = await response.text()
                raise Exception(f"DELETE {entity_name}({key}) failed: {response.status} - {error_text}")
    ...
    async def call_action(self, action_name: str, parameters: Optional[Dict[str, Any]] = None,
        ...
        """
        session = await self.session_manager.get_session()
        url = QueryBuilder.build_action_url(self.base_url, action_name, entity_name, entity_key)
