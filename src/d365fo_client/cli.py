...
class CLIManager:
    ...
    async def execute_command(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if not config.base_url:
                print(format_error_message("Base URL is required. Use --base-url or configure a profile."))
                return 1
    ...
    async def _handle_entity_get(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            # Build query options
            query_options = None
            if any([getattr(args, 'select', None), getattr(args, 'filter', None), getattr(args, 'top', None), getattr(args, 'orderby', None)]):
                query_options = QueryOptions(
                    select=getattr(args, 'select', '').split(',') if getattr(args, 'select', '') else None,
                    filter=getattr(args, 'filter', None),
                    top=getattr(args, 'top', None),
                    orderby=getattr(args, 'orderby', '').split(',') if getattr(args, 'orderby', '') else None
                )
    ...
    async def _handle_entity_delete(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if not getattr(args, 'confirm', False):
                response = input(f"Are you sure you want to delete {entity_name} with key '{key}'? (y/N): ")
    ...
    async def _handle_action_list(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if entity:
                ...
                actions = [action for action in actions if entity.lower() in action.lower()]
    ...
    async def _handle_action_call(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            print(format_success_message(f"Action '{action_name}' executed successfully"))
            output = self.output_formatter.format_output(result)
            print(output)
            return 0
        ...
        try:
            ...
            for name, profile in profiles.items():
                profile_info = {
                    ...
                    'default': '✓' if default_profile and default_profile.name == name else ''
                }
                profile_list.append(profile_info)
            
            output = self.output_formatter.format_output(profile_list, ['name', 'base_url', 'auth_mode', 'default'])
            print(output)
            return 0
    ...
    def _handle_config_create(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if not base_url:
                print(format_error_message("--base-url is required when creating a profile"))
                return 1
            ...
            print(format_success_message(f"Profile '{profile_name}' created successfully"))
            return 0
    ...
    def _handle_config_delete(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if success:
                print(format_success_message(f"Profile '{profile_name}' deleted successfully"))
                return 0
            else:
