...
async def resolve_labels_generic(
    ...
    if isinstance(obj_or_list, list):
        ...
        logger.debug(f"Collected {len(label_ids)} unique label IDs from list: {list(label_ids)}")
        
        # Resolve all labels in batch
        label_texts = await _resolve_labels_batch(list(label_ids), label_operations, language)
        
        logger.debug(f"Successfully resolved {len(label_texts)} labels out of {len(label_ids)} requested")
        ...
        logger.debug(f"Completed label resolution for list with {len(obj_or_list)} objects")
        return obj_or_list
    ...
    # Handle single object
        ...
        logger.debug(f"Collected {len(label_ids)} label IDs from object: {list(label_ids)}")
...
def _collect_label_ids_from_object(obj: Any, label_ids: set) -> None:
    ...
    nested_attrs = [
        'properties', 'members', 'navigation_properties', 'property_groups', 
        'actions', 'parameters', 'constraints', 'enhanced_properties'
    ]
    ...
    for attr_name in nested_attrs:
        if hasattr(obj, attr_name):
            ...
            if isinstance(nested_obj, list):
                if nested_obj:
                    logger.debug(f"Recursively collecting from {len(nested_obj)} items in {attr_name}")
    ...
    if collected_count > 0:
        logger.debug(f"Collected {collected_count} new label IDs from {obj_type} object")
    else:
        logger.debug(f"No new label IDs found in {obj_type} object")


async def _resolve_labels_batch(label_ids: List[str], label_operations: LabelOperations, language: str) -> Dict[str, str]:
    """
    ...
    logger.debug(f"Starting batch resolution of {len(label_ids)} label IDs in language '{language}'")
    ...
    try:
        ...
        logger.info(f"Batch label resolution completed: {successful_resolutions} successful, {failed_resolutions} failed out of {len(label_ids)} total")
...
def _apply_labels_to_object(obj: Any, label_texts: Dict[str, str]) -> None:
    ...
    if (hasattr(obj, 'label_id') and hasattr(obj, 'label_text') and 
        obj.label_id and obj.label_id in label_texts):
        old_text = obj.label_text
        obj.label_text = label_texts[obj.label_id]
        labels_applied += 1
        logger.debug(f"Applied label to {obj_type}: '{obj.label_id}' -> '{obj.label_text}' (was: {old_text})")
    elif hasattr(obj, 'label_id') and obj.label_id:
        if not hasattr(obj, 'label_text'):
            logger.debug(f"{obj_type} has label_id '{obj.label_id}' but no label_text attribute")
        elif obj.label_id not in label_texts:
            logger.debug(f"{obj_type} has label_id '{obj.label_id}' but no resolved text available")
    ...
    for attr_name in nested_attrs:
        if hasattr(obj, attr_name):
            ...
            if isinstance(nested_obj, list):
                if nested_obj:
                    logger.debug(f"Applying labels to {len(nested_obj)} items in {attr_name}")
                    for i, item in enumerate(nested_obj):
