...
class MetadataDatabase:
    ...
    async def initialize(self):
        ...
        if aiosqlite is None:
            raise MetadataError(
                "aiosqlite is required for metadata caching. Install with: pip install aiosqlite"
            )
    ...
    async def get_or_create_environment(self, base_url: str) -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            cursor = await db.execute(
                ...
                (
                    base_url,
                    self._extract_environment_name(base_url),
    ...
    async def create_version(
        self, environment_id: int, version_info: MetadataVersionInfo
    ) -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            cursor = await db.execute(
                ...
                (
                    environment_id,
                    version_info.version_hash,
                    version_info.application_version,
                    version_info.platform_version,
                    json.dumps(version_info.package_info or []),
                    datetime.now(timezone.utc),
                ),
            )
            await db.commit()
    ...
    async def rebuild_fts_index(self, environment_id: int):
        ...
        if not version_info:
            logger.warning(
                "No active metadata version found, cannot rebuild search index"
            )
            return
    ...
    async def get_label(
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if row:
                ...
                return LabelInfo(id=label_id_db, language=language_db, value=value)
            return None
            ...
            if environment_id:
                ...
                if version_info:
                    ...
                    if version_row:
                        ...
                        stats["active_version_data_entities"] = (
                        ...
                        stats["active_version_public_entities"] = (
                            await cursor.fetchone()
                        )[0]
                        ...
                        stats["active_version_enumerations"] = (
                            await cursor.fetchone()
                        )[0]
                        ...
                        stats["active_version_properties"] = (await cursor.fetchone())[
                            0
                        ]
...
class MetadataCache:
    ...
    def _initialize_caches(self):
        ...
        if TTLCache is not None:
            self._memory_cache = TTLCache(
                maxsize=self.max_memory_size, ttl=self.cache_ttl
            )
        else:
    ...
    async def initialize(self):
        ...
        self._environment_id = await self._database.get_or_create_environment(
    ...
    async def get_entity(
        self, entity_name: str, entity_type: str = "public"
    ) -> Optional[Union[DataEntityInfo, PublicEntityInfo]]:
    ...
    async def _get_data_entity_from_db(
    ...
    async def _get_public_entity_from_db(
        self, entity_name: str
        ...
        async with aiosqlite.connect(self._database.db_path) as db:
            ...
            entity.navigation_properties = await self._get_navigation_properties(
                db, entity_id
            )
    ...
    async def _get_entity_properties(
        self, db: Any, entity_id: int
    ) -> List[PublicEntityPropertyInfo]:
    ...
    async def _get_navigation_properties(
        self, db: Any, entity_id: int
    ) -> List[NavigationPropertyInfo]:
    ...
    async def _get_relation_constraints(
        self, db: Any, nav_prop_id: int
    ) -> List[RelationConstraintInfo]:
    ...
    async def _get_property_groups(
        self, db: Any, entity_id: int
    ) -> List[PropertyGroupInfo]:
        ...
        async for row in cursor:
            ...
            group = PropertyGroupInfo(name=name, properties=properties)
            groups.append(group)
    ...
    async def _get_entity_actions(self, db: Any, entity_id: int) -> List[ActionInfo]:
        ...
        async for row in cursor:
            (
                ...
                field_lookup,
                return_type_name,
                return_is_collection,
                return_odata_xpp_type,
            ) = row
    ...
    async def _get_action_parameters(
        ...
        async for row in cursor:
            ...
            param = ActionParameterInfo(
                name=name, type=param_type, parameter_order=parameter_order
            )
        ...
        with self._lock:
            for label in labels:
                cache_key = self._build_cache_key(
                    "label", label.id, language=label.language
                )
    ...
    async def search_data_entities(
        ...
        logger.debug(
            f"Database lookup for data entities search: {pattern}, found {len(entities)} entities"
        )

        return entities

    async def _search_data_entities_from_db(
        self,
        pattern: str = "",
        entity_category: Optional[str] = None,
        data_service_enabled: Optional[bool] = None,
        ...
        if pattern and re.search(r"[.*+?^${}()|[\]\\]", pattern):
            try:
                ...
                logger.warning(
                    f"Invalid regex pattern '{pattern}', using substring matching"
                )
    ...
    async def search_public_entities(
        ...
        if self._disk_cache:
            try:
                ...
                if entities is not None:
                    logger.debug(
                        f"Disk cache hit for public entities search: {pattern}"
                    )
                    # Store in memory cache
                    if self._memory_cache:
                        self._memory_cache[cache_key] = entities
        ...
        logger.debug(
            f"Database lookup for public entities search: {pattern}, found {len(entities)} entities"
        )
        return entities

    async def _search_public_entities_from_db(
        self,
        pattern: str = "",
        is_read_only: Optional[bool] = None,
        configuration_enabled: Optional[bool] = None,
    ...
    async def search_public_enumerations(
        ...
        if self._memory_cache:
            ...
            if enumerations is not None:
                logger.debug(
                    f"Memory cache hit for public enumerations search: {pattern}"
                )
                return enumerations
        ...
        if self._disk_cache:
            try:
                ...
                if enumerations is not None:
                    logger.debug(
                        f"Disk cache hit for public enumerations search: {pattern}"
                    )
                    # Store in memory cache
                    if self._memory_cache:
        ...
        if self._disk_cache:
            try:
                ...
                logger.warning(
                    f"Failed to cache public enumerations search to disk: {e}"
                )

        logger.debug(
            f"Database lookup for public enumerations search: {pattern}, found {len(enumerations)} enumerations"
        )
        return enumerations

    async def _search_public_enumerations_from_db(
        self, pattern: str = ""
    ) -> List[EnumerationInfo]:
        ...
        enumerations = []
        for row in rows:
            enum = EnumerationInfo(name=row[0], label_id=row[1])
    ...
    async def _get_enumeration_from_db(
        self, enumeration_name: str
    ) -> Optional[EnumerationInfo]:
        ...
        async with aiosqlite.connect(self._database.db_path) as db:
            ...
            enum = EnumerationInfo(name=name, label_id=label_id)
    ...
    async def search_actions(
        ...
        # L3: Database
        actions = await self._search_actions_from_db(
            pattern, entity_name, binding_kind, is_function
        )
        ...
        logger.debug(
            f"Database lookup for actions search: {pattern}, found {len(actions)} actions"
        )
        return actions

    async def _search_actions_from_db(
        self,
        pattern: str = "",
        entity_name: Optional[str] = None,
        binding_kind: Optional[str] = None,
        ...
        if not entity_name:
            unbound_where_clause = (
                " AND ".join(unbound_where_conditions)
                if unbound_where_conditions
                else "1=1"
            )
            unbound_query = f"""
                SELECT ea.id, ea.name, ea.binding_kind, ea.field_lookup,
        ...
        if pattern and re.search(r"[.*+?^${}()|[\]\\]", pattern):
            try:
                ...
                actions = [
                    action for action in actions if regex_pattern.search(action.name)
                ]
            except re.error:
    ...
    async def _get_action_parameters_for_search(
        self, db: Any, action_id: int
    ) -> List[ActionParameterInfo]:
        ...
        async for row in cursor:
            ...
            parameter = ActionParameterInfo(
                name=name, type=action_type, parameter_order=parameter_order
            )
    ...
    async def get_action_info(
        self, action_name: str, entity_name: Optional[str] = None
    ) -> Optional[ActionInfo]:
...
__all__ = ["MetadataDatabase", "MetadataCache", "MetadataSearchEngine"]
