    ...
    def from_dict(cls, data: Dict[str, Any]) -> "CredentialSource":
        ...
        if source_type == "environment":
            ...
            return KeyVaultCredentialSource(
                ...
                client_id_secret_name=data.get(
                    "client_id_secret_name", "D365FO_CLIENT_ID"
                ),
                client_secret_secret_name=data.get(
                    "client_secret_secret_name", "D365FO_CLIENT_SECRET"
                ),
                tenant_id_secret_name=data.get(
                    "tenant_id_secret_name", "D365FO_TENANT_ID"
                ),
...
class EnvironmentCredentialProvider(CredentialProvider):
    ...
    async def get_credentials(self, source: CredentialSource) -> Tuple[str, str, str]:
        ...
        if not isinstance(source, EnvironmentCredentialSource):
            raise ValueError(
                f"Expected EnvironmentCredentialSource, got {type(source)}"
            )
        ...
        logger.debug(
            f"Retrieved credentials from environment variables: {source.client_id_var}, {source.client_secret_var}, {source.tenant_id_var}"
        )
...
class KeyVaultCredentialProvider(CredentialProvider):
    ...
    async def get_credentials(self, source: CredentialSource) -> Tuple[str, str, str]:
        ...
        try:
            ...
            client_secret_secret = secret_client.get_secret(
            ...
            if not all([client_id, client_secret, tenant_id]):
                raise ValueError(
                    "One or more secrets retrieved from Key Vault are empty"
                )
            ...
            logger.error(
                f"Failed to retrieve credentials from Key Vault {source.vault_url}: {e}"
            )
    ...
    def _get_secret_client(self, source: KeyVaultCredentialSource) -> SecretClient:
        ...
        if source.keyvault_auth_mode == "default":
            ...
            logger.debug(
                f"Using default credentials for Key Vault authentication: {vault_url}"
            )
        elif source.keyvault_auth_mode == "client_secret":
            if not all(
                [
                    source.keyvault_client_id,
                    source.keyvault_client_secret,
                    source.keyvault_tenant_id,
                ]
            ):
                raise ValueError(
                    "Key Vault client_secret authentication requires keyvault_client_id, keyvault_client_secret, and keyvault_tenant_id"
                )

            assert (
                source.keyvault_client_id
                and source.keyvault_client_secret
                and source.keyvault_tenant_id
            )  # For type checker
            ...
            logger.debug(
                f"Using client secret credentials for Key Vault authentication: {vault_url}"
            )
        else:
            raise ValueError(
                f"Invalid keyvault_auth_mode: {source.keyvault_auth_mode}. Must be 'default' or 'client_secret'"
            )
...
class CredentialManager:
    ...
    async def get_credentials(self, source: CredentialSource) -> Tuple[str, str, str]:
        ...
        if cache_key in self._credential_cache:
            ...
            if cached.is_valid_for_source(source_hash):
                logger.debug(
                    f"Using cached credentials for source type: {source.source_type}"
                )
                ...
                logger.debug(
                    f"Expired cached credentials removed for source type: {source.source_type}"
                )
        ...
        if not provider:
            raise ValueError(
                f"Unsupported credential source type: {source.source_type}"
            )
        ...
        logger.debug(
            f"Retrieved and cached credentials from source type: {source.source_type}"
        )
        return client_id, client_secret, tenant_id
    ...
    def clear_cache(self, source_type: Optional[str] = None) -> None:
        ...
        if source_type is None:
            ...
            keys_to_remove = [
                key
                for key in self._credential_cache.keys()
    ...
    def get_cache_stats(self) -> Dict[str, int]:
        ...
        expired_count = sum(
            1 for cached in self._credential_cache.values() if cached.is_expired()
        )
